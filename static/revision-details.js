(() => {
  ...
  function trendHtml(value) {
    ...
    // For shortage, growth is bad (red), decline is good (green).
  ...
  function docHtml(doc) {
    const diff = Number(doc.lineControlDifference || 0);
    return `<div class="rev-doc">
      <div class="rev-doc-head">
        ...
        <div class="rev-doc-metric ${diff <= .01 ? 'ok' : 'neg'}"><span>Контроль строк</span><b>${money(diff)}</b></div>
      </div>
  ...
  function detailHtml(data, historyItem) {
    ...
    return `<div class="rev-detail-wrap">
      ...
      <div class="rev-validation"><b>* Δ количество</b> — движение количества из TRANSACTIONS OLAP. Оно помогает видеть направление и масштаб расхождения, но это ещё не отдельные колонки «книжное количество» и «фактическое количество» из окна iikoChain. <b>Контроль строк 0 ₸</b> означает, что итог документа совпадает с суммой его строк внутри полученного OLAP. Независимый итог самого окна iikoChain этот отчёт пока не отдаёт.</div>
    </div>`;
  }
